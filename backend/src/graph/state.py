...
class VideoAuditState(TypedDict):
    ...
    #firstly video will be downloaded and temporarily stored in a local path, then the path will be stored in this field.And then we are going to extract the video matadata.
    ...
    # Appends system-level errors (e.g., API timeouts or download failures) without halting execution logic.So we want all those errors to be collected in a list and then we can log them or send them to the observability system.
