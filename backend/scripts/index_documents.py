...
load_dotenv(override=True) #this line loads environment variables from a .env file, overriding any existing environment variables with the same name
...
logger = logging.getLogger("indexer")
 ...
 #this function will read the pdf's from the backend/data folder, chunk them into smaller pieces, and upload them to Azure AI Search for indexing.
def index_docs():
    """
    ...
    logger.info("=" * 60)  # this line prints a separator for better readability in the logs
    ...
    logger.info(f"Found {len(pdf_files)} PDFs to process: {[os.path.basename(f) for f in pdf_files]}")  # this line logs the number of PDFs found and their filenames for transparency and debugging purposes
    all_splits = []
...
#This if block acts as a "gatekeeper." It tells Python: "Only run the index_docs() function if I am running this file directly from the terminal. Do not run it automatically if I am just importing the function to use elsewhere."
